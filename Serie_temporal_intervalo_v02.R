...
for (i in 1:length(my_dt)) {
  ...
  previsao[[i]] = forecast(serie[[i]],h=dias_uteis, level=95)
...
fit1=  arima(ts(my_dt[[35]]$VL_QUOTA),order=c(0,1,1))
summary(fit1)
...
# 
#Analise fundo ações agro 55
serie[[55]]
# Apos analise foi mantido o modelo selecionado pelo auto arima



#Analise fundo RF FIC BRASIL ATIVA 51
serie[[51]]
ts(my_dt[[51]]$VL_QUOTA) %>% autoplot()
ts(diff(my_dt[[51]]$VL_QUOTA,differences=1)) %>% autoplot()
ts(diff(my_dt[[51]]$VL_QUOTA,differences=2)) %>% autoplot()
tseries::adf.test (ts(diff(my_dt[[51]]$VL_QUOTA,differences=1)),k=0) 
tseries::adf.test (ts(diff(my_dt[[51]]$VL_QUOTA,differences=2)),k=0)

acf(diff(ts(my_dt[[51]]$VL_QUOTA), differences=1))

pacf(diff(ts(my_dt[[51]]$VL_QUOTA), differences=1))

acf(diff(ts(my_dt[[51]]$VL_QUOTA), differences=2))

pacf(diff(ts(my_dt[[51]]$VL_QUOTA), differences=2))

# A partir da analise das auto correlações vemos que temos excesso de diferenças quando usamos 2 
# entao optaremos por 1 diferença so 
# o arima escolhido primariamente sera(0,1,1)
# que sera comparado ao (0,1,0)


fit3=  arima(ts(my_dt[[51]]$VL_QUOTA),order=c(0,1,1))
summary(fit3)

pred3 = forecast(fit3,h=dias_uteis, level=95)
pred3

 
# Optamos pelo arima(0,1,1)




# Analise fundo RF FI ALIANCA posição 5 


# alterando a ordem do arima
serie[[5]]
#(1,2,1) pelo auto arima
ts(my_dt[[5]]$VL_QUOTA) %>% autoplot()
ts(diff(my_dt[[5]]$VL_QUOTA,differences=1)) %>% autoplot()
ts(diff(my_dt[[5]]$VL_QUOTA,differences=2)) %>% autoplot()
tseries::adf.test (ts(diff(my_dt[[5]]$VL_QUOTA,differences=1)),k=0) 
tseries::adf.test (ts(diff(my_dt[[5]]$VL_QUOTA,differences=2)),k=0)

acf(diff(ts(my_dt[[5]]$VL_QUOTA), differences=1))

pacf(diff(ts(my_dt[[5]]$VL_QUOTA), differences=1))

acf(diff(ts(my_dt[[5]]$VL_QUOTA), differences=2))

pacf(diff(ts(my_dt[[5]]$VL_QUOTA), differences=2))




fit4=  arima(ts(my_dt[[5]]$VL_QUOTA),order=c(1,2,1))
summary(fit4)

pred4 = forecast(fit4,h=dias_uteis, level=95)
pred4

#Sera utilizado arima(1,2,1)
# Verificar por que essa previsao está diferente da feita antes , sendo que o modelo é igual
...
## FUndo RF FLUXO FIC 24 
  
serie[[24]]
ts(my_dt[[24]]$VL_QUOTA) %>% autoplot()
ts(diff(my_dt[[24]]$VL_QUOTA,differences=1)) %>% autoplot()
ts(diff(my_dt[[24]]$VL_QUOTA,differences=2)) %>% autoplot()
tseries::adf.test (ts(diff(my_dt[[24]]$VL_QUOTA,differences=1)),k=0) 
tseries::adf.test (ts(diff(my_dt[[24]]$VL_QUOTA,differences=2)),k=0)

acf(diff(ts(my_dt[[24]]$VL_QUOTA), differences=1))

pacf(diff(ts(my_dt[[24]]$VL_QUOTA), differences=1))

acf(diff(ts(my_dt[[24]]$VL_QUOTA), differences=2))

pacf(diff(ts(my_dt[[24]]$VL_QUOTA), differences=2))
...
fit5=  arima(ts(my_dt[[24]]$VL_QUOTA),order=c(1,2,1))
summary(fit5)

pred5 = forecast(fit5,h=dias_uteis, level=95)
pred5
...
# RF DISPONIBILIDADES posição 32


serie[[32]]
#(0,2,0) pelo auto arima

ts(my_dt[[32]]$VL_QUOTA) %>% autoplot()
ts(diff(my_dt[[32]]$VL_QUOTA,differences=1)) %>% autoplot()
ts(diff(my_dt[[32]]$VL_QUOTA,differences=2)) %>% autoplot()

tseries::adf.test (ts(diff(my_dt[[32]]$VL_QUOTA,differences=1)),k=0) 
tseries::adf.test (ts(diff(my_dt[[32]]$VL_QUOTA,differences=2)),k=0)

acf(diff(ts(my_dt[[32]]$VL_QUOTA), differences=1))

pacf(diff(ts(my_dt[[32]]$VL_QUOTA), differences=1))

acf(diff(ts(my_dt[[32]]$VL_QUOTA), differences=2))

pacf(diff(ts(my_dt[[32]]$VL_QUOTA), differences=2))
...
fit6=  arima(ts(my_dt[[32]]$VL_QUOTA),order=c(0,2,0))
summary(fit6)

pred6 = forecast(fit6,h=dias_uteis, level=95)
pred6
...
serie[[11]]
#(0,1,0) pelo auto arima

ts(my_dt[[11]]$VL_QUOTA) %>% autoplot()
ts(diff(my_dt[[11]]$VL_QUOTA,differences=1)) %>% autoplot()
tseries::adf.test (ts(diff(my_dt[[11]]$VL_QUOTA,differences=1)),k=0) 

acf(diff(ts(my_dt[[11]]$VL_QUOTA), differences=1))

pacf(diff(ts(my_dt[[11]]$VL_QUOTA), differences=1))
...
fit7=  arima(ts(my_dt[[11]]$VL_QUOTA),order=c(0,1,0))
summary(fit7)

pred7 = forecast(fit7,h=dias_uteis, level=95)
pred7
...
# BB PREVI TP IPCA (VERTICE)
#Sem mudança para o que já foi feito antes

# AÇÔES GOVERNANÇA posição 12
#Sem mudança para o que já foi feito antes

# AÇÔES BOLSAS EUROPEIAS posição 53
# Sem mudança para o que já foi feito antes 


#
# FICA AÇÔES LIVRE QUANTITATIVO posição 46
# Sem alterações pro que foi modelado anteriormente 

# Analise do fundo titulos publicos posição 56
# Apos analise foi mantido o modelo selecionado pelo auto arima 
...
# Mudando as previsões
  ...
  Previsao[35] = tail(pred$lower,1)
  
 


  # AÇÕES ALOC INVESTIM EXTERIOR
  Previsao[43] = tail(pred2$lower,1)

  
  #RF FIC BRASIL ATIVA
  Previsao[51] = tail(pred3$lower,1)

  
  
  #RF FI ALIANÇA
  Previsao[5] = tail(pred4$lower,1)
  

  
  # RF FLUXO FIC 
  Previsao[24] = tail(pred5$lower,1)
  ...
  Previsao[32] = tail(pred6$lower,1)
  
  # AÇÕES RETORNO TOTAL
  
  Previsao[11] = tail(pred7$lower,1)
